...
def nn_model():
    model = Sequential()
    model.add(Dense(512, kernel_initializer='normal',input_dim = 170, activation='selu'))
    ...
    #model.add(Dense(256, kernel_initializer='normal',activation='relu'))kernel_regularizers='l2'
    model.add(Dense(1, kernel_initializer='normal',activation='linear'))
    model.compile(loss='mean_absolute_error', optimizer=Adam(learning_rate=0.0003), metrics=['RootMeanSquaredError'])
    return model
...
def get_estimator():
    ...
    cat_columns_boost = ['ori', 'dest','is_holiday', 'year', 'month', 'day', 'weekday', 'week']
    #date_columns_boost = ['import_day']
    num_columns_boost = ['WeeksToDeparture', 'std_wtd', 'distance',
                   'departures_performed','passengers','import_route',
                   'import_month', 'load', 'cancel_rate', 'fuel_price',
    ...
    cat_encoder_boost = OneHotEncoder(handle_unknown="ignore")
    num_encoder_boost = StandardScaler()
    preprocessor_boost = make_column_transformer(
        (cat_encoder_boost, cat_columns_boost), 
        (num_encoder_boost, num_columns_boost),
    ...
    boost_reg = CatBoostRegressor(n_estimators = 5000, learning_rate=0.05, max_depth=6, verbose=False)
 
 
    cat_columns_nn = ['ori', 'dest', 'is_holiday', "weekday", 'year', 'month', 'day', 'week']
    ...
    nn_reg = KerasRegressor(build_fn=nn_model, epochs=60, batch_size=16, verbose=False)
